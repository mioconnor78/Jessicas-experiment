...
CT1 <- lme(I(log(chla)) ~ week, random = ~ 1 | Tank, data=alldata, method="ML", na.action=na.omit) 
...
## 0.5. Temporal Results: Pp size over time (8 weeks)

```{r, echo = FALSE, Fig_S2.01b, fig.width=5, fig.height=3}
...
CT2 <- lme(I(log(chla)) ~ temp.wk, random = ~ 1 | Tank, data=alldata, method="ML", na.action=na.omit) 
...
```{r}
## TC mass normalized
plot(log(alldata1$mba1) ~ log(alldata1$chla))
plot(log(alldata1$Omba1) ~ log(alldata1$chla))
plot(log(alldata1$Omba1) ~ log(alldata1$O.N))
plot(log(alldata1$pp.N) ~ log(alldata1$chla))
plot(log(alldata1$Mb) ~ log(alldata1$chla))
## so there are good relationships between cell number and estimated mass (which is based on the number, size and shape of cells). But no agreement between [chla] and estimated cell mass or vol, for cyanos, non-cyanos, or total. What to do...

TCm <- as.tibble(alldata1) %>%
group_by(week) %>%
select(., week, power, trophic.level, mba1) %>%
spread(., trophic.level, mba1) %>%
left_join(., temps.pwr, by = c("week", "power")) %>%
mutate(., invTavg = 1/((avgTemp + 273)*k)) %>%
mutate(., TCMb = log(PZN/PZ)) 
...
### 1.2 duplicate TC analysis with TCmb
#```{r, echo = FALSE}
TCmodb <- lme(TCMb ~ I(invTavg-mean(invTavg))*week, random = ~ 1|power + 1|week, data=TCvol, method = "REML", na.action = na.omit)
TCmodc <- lme(TCMb ~ I(invTavg-mean(invTavg))*week, random = ~ 1|power, data=TCvol, method = "REML", na.action = na.omit)
...
#```{r}
## fixed effects model:
TCVmodb <- lme(TCMb ~ 1 + I(invTavg-mean(invTavg))*week, random = ~ 1|power, data=TCvol, method="ML", na.action=na.omit)
TCVmodc <- lme(TCMb ~ 1 + I(invTavg-mean(invTavg)) + week, random = ~ 1|power, data=TCvol, method="ML", na.action=na.omit)
TCVmodd <- lme(TCMb ~ 1 + I(invTavg-mean(invTavg)), random = ~ 1|power, data=TCvol, method="ML", na.action=na.omit)
TCVmode <- lme(TCMb ~ 1 + week, random = ~ 1|power, data=TCvol, method="ML", na.action=na.omit)
TCVmodf <- lme(TCMb ~ 1 + week, random = ~ 1|power, data=TCvol, method="ML", na.action=na.omit)

TCVres <- data.frame(model.sel(TCVmodb, TCVmodc, TCVmodd, TCVmode, TCVmodf))
```

```{r Fig_S2.04b TC Strength, fig.width=4, fig.height=3}
TCM <- ggplot(data = TCm, aes(x = I(invTavg-mean(invTavg)), y = TCMb, ymin = 0, group = week, shape = as.factor(week), alpha = week)) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major = element_blank(),
        legend.text = element_text(size = 6), 
  ...
  ylab(bquote(~"Trophic Cascade"~"ln(Mb"[+P]~")/(Mb"[-P]~")")) +
...
## 2.1.1b Phytoplankton metabolic biomass candidate models
```{r, echo=FALSE}
#### Phytoplankton coefficients for Figure 2
data <- alldata1
modPBF <- lme(log(mba1) ~ 1 + I(invTi - invTT) + trophic.level + trophic.level*I(invTi - invTT) + I(invTT - mean(invTT)) + I(invTi - invTT)*I(invTT - mean(invTT)), random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit) 
modPB8 <- lme(log(mba1) ~ 1 + trophic.level*I(invTi - invTT) + trophic.level*I(invTT - mean(invTT)), random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB7 <- lme(log(mba1) ~ 1 + I(invTi - invTT) + trophic.level + trophic.level*I(invTT - mean(invTT)), random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB6 <- lme(log(mba1) ~ 1 + trophic.level*I(invTi - invTT), random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB5 <- lme(log(mba1) ~ 1 + I(invTi - invTT) + trophic.level, random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB4 <- lme(log(mba1) ~ 1 + I(invTi - invTT)*I(invTT - mean(invTT)), random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB3 <- lme(log(mba1) ~ 1 + I(invTi - invTT) + I(invTT - mean(invTT)), random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB2 <- lme(log(mba1) ~ 1 + I(invTi - invTT), random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB1 <- lme(log(mba1) ~ 1 + trophic.level, random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
modPB0 <- lme(log(mba1) ~ 1, random = ~ 1 | Tank, data=data, method="ML", na.action=na.omit)
...
```{r, NEP model selection b, echo=FALSE}
...
```{r, NEP model selection a, echo=FALSE}

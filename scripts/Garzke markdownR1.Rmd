...
```{r, echo = FALSE, Fig_S2.01, fig.width=5, fig.height=3}
mba1.time <- ggplot(data = alldata1, aes(x = week, y = log(mba1), ymin = 0, shape = trophic.level, alpha = trophic.level)) +
  theme_bw() +
  #theme(legend.position = "none") +
  geom_point() +
  facet_grid(trophic.level ~ .) +
...
```{r}
data2 <- alldata %>%
  left_join(., ppsizes, by = c("Tank" = "tank", "week"))
```

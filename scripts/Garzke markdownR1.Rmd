...
```{r Plot Chla over time, echo = FALSE, Fig_S2.01,fig.width=5, fig.height=3}
...
```{r Plot Chla over temps, echo = FALSE, Fig_S2.02, fig.width=5, fig.height=3}
...
## Trophic cascade (TC) is defined as log(PZN/PZ) for Chlorophyll a
```{r Calculate TC, echo = FALSE, Fig_S2.03, fig.width=2, fig.height=2}
...
```{r lmer ranef model comparison, echo = FALSE}
...
```{r nlme model comparisons, echo = FALSE}
## in nlme
#library(nlme)
...
```{r nlme fixed effects comparison}
## fixed effects model:
...
```{r Model estimates for TC figure}
...
TC <- ggplot(data = TCchla, aes(x = I(invTavg-mean(invTavg)), y = TC, ymin = 0)) +
  theme_bw() +
  #theme(legend.position = "none") +
  theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank(),  
        legend.text=element_text(size=6), 
  ...
  geom_point(aes(group = week, shape = as.factor(week), alpha = week)) +
  scale_shape_manual(values = c(1,2,22,23,8,15,16,17)) +
  scale_alpha("week", guide = "none")

TC +
  geom_smooth(data = TCchla[(TCchla$week == '2'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals2, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'gray80') +
  geom_smooth(data = TCchla[(TCchla$week == '3'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals3, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'gray70') +
  geom_smooth(data = TCchla[(TCchla$week == '4'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals4, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'gray60') +
  geom_smooth(data = TCchla[(TCchla$week == '5'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals5, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'gray50') +
  geom_smooth(data = TCchla[(TCchla$week == '6'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals6, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'gray40') +
  geom_smooth(data = TCchla[(TCchla$week == '7'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals7, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'gray30') +
  geom_smooth(data = TCchla[(TCchla$week == '8'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals8, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'gray20') +
  geom_smooth(data = TCchla[(TCchla$week == '9'),], aes(x = I(invTavg-mean(invTavg)), y = TCvals9, ymin = 0), method = "lm", se = FALSE, inherit.aes = FALSE, formula = y ~ x, color = 'black') 
```

...
m2a <- lmer(log(size) ~ invTT*trophic.level*taxon + (1|Tank), data = data.S)
#summary(m2a)
#shapiro.test(resid(m2a))

m2b <- lmer(log(size) ~ invTT*taxon*trophic.level + (1|Tank), data = data.S)
m2j <- lmer(log(size) ~ invTT*taxon + trophic.level + (1|Tank), data = data.S)
m2c <- lmer(log(size) ~ invTT + trophic.level*taxon + (1|Tank), data = data.S)
m2d <- lmer(log(size) ~ invTT + trophic.level + (1|Tank), data = data.S)
m2k <- lmer(log(size) ~ invTT*trophic.level + (1|Tank), data = data.S)
m2m <- lmer(log(size) ~ invTT*trophic.level + taxon + (1|Tank), data = data.S)
m2e <- lmer(log(size) ~ invTT + taxon + (1|Tank), data = data.S)
m2l <- lmer(log(size) ~ invTT*taxon + (1|Tank), data = data.S)
m2f <- lmer(log(size) ~ invTT + (1|Tank), data = data.S)
m2g <- lmer(log(size) ~ trophic.level*taxon + (1|Tank), data = data.S)
m2h <- lmer(log(size) ~ taxon + (1|Tank), data = data.S)
m2i <- lmer(log(size) ~ trophic.level + (1|Tank), data = data.S)

...
```{r resid plot of linear model, echo=FALSE, fig.width=4,fig.height=4,fig.cap="\\label{fig:figs} Resid plot of linear model zooplankton (Number / 10L) over all tanks and weeks."}
qqnorm(resid(m2))
qqline(resid(m2)) ## ok, shapiro test + this plot show that the linear model is not right.
```

```{r poison errors, echo = FALSE}
## third dry: glm with poisson errors.
...
```{r N plot, fig.width=4,fig.height=4,fig.cap="\\label{fig:figs} Total Zooplankton abundance and modeled temperature dependence from negative binomial regression."}
...
```{r Daphnia abundance, fig.width=4,fig.height=4}

...
```{r specify PP data, Fig_S2.1,fig.width=3, fig.height=3}
data <- alldata1
hist(data$chla)
hist(log(data$chla))
...
```{r PP.plot2}
PP.plot2 <- ggplot(data = data, aes(x = -invTi, y = log(chla), ymin = 0, ymax = 3)) + #, xmin = -40.2, xmax = -38.2
  theme_bw() +
 ...
 geom_line(data = subset(data, trophic.level == "PZ"), aes(x = -invTT, y = PBvalsPZ), color = 'black', linetype = 3, size = 1.5) +
 geom_line(data = subset(data, trophic.level == "PZN"), aes(x = -invTT, y = PBvalsPZN), color = 'black', linetype = 2, size = 1.5) +
  scale_linetype_identity() +
  ylab("ln(ug Chl a / L)") +
  labs(title = "A. Phytoplankton Biomass [Chla]", size = 3) +
  ...
  ggsave("Fig2A.png", device = "png", width = 4, height = 4)
```

`r fig_nums("Fig_S1.2")`

```{r Figure 2A main text, Fig_S1.2, fig.width=4, fig.height=3}
PP.plot2
```

## working plot for coefficients. eventually this will be below an will draw on a table of all the coefs.
```{r working fig 3, eval=FALSE, include=FALSE}
plot(NULL,                                
     xlim = c(-6, 7),                        	
...
```{r Figure 2B main text, fig.width=4, fig.height=3}
...
TC <- ggplot(data = TCchla, aes(x = -I(invTavg-mean(invTavg)), y = log(TC), ymin = 0, group = week, shape = as.factor(week), alpha = week)) +
  ...
  scale_alpha("week", guide = "none") +
  scale_shape_manual(guide = guide_legend(title = "Week"), values = c(1,2,22,23,8,15,16,17)) +
  labs(title = "B. Trophic Cascade Strength", size = 3) +
  ylab(bquote(~"ln(Chl a"[+AGP]~")/(Chl a"[-AG]~")")) +
  #scale_x_continuous("Temperature [(1/kTi) - (1/kTi)]", sec.axis = sec_axis(~((1/(k*+.))-273), name = xlab))
  xlab("Temperature [(1/kTi) - (1/kTi)]") 
...
#ggsave("Fig2B.png", device = "png", width = 4, height = 3)
```
...
```{r NEP.plot2}
NEP.plot2 <- ggplot(data = data1, aes(x = -invTi, y = log(NPP2), ymin = -7, ymax = -5)) + #, xmin = -40.2, xmax = -38.2
  theme_bw() +
  theme(legend.position = "none",
        panel.grid.minor = element_blank(), 
        panel.grid.major = element_blank(),  
 ...
 geom_line(data = subset(data1, trophic.level == "PZN"), aes(x = -invTT, y = NvalsPZN), color = 'black', linetype = 2, size = 1.5) +
  ...
  labs(title = "A. Net Ecosystem Production (NEP)", size = 3) +
  annotate("text", x = c(-39,-39,-39), y = c(-6.5, -6.7, -6.9), label = c("solid = A", "dotted = AG", "dashed = AGP"), size = 4) +
  scale_x_continuous("Temperature (1/kTi)", sec.axis = sec_axis(~(-(1/(k*+.))-273), name = xlab))

  ggsave("Fig3A.png", device = "png", width = 4, height = 4)
```       
...
```{r ER.plot2}
ER.plot2 <- ggplot(data = data2, aes(x = -invTi, y = log(ER2), ymin = -7, ymax = -5)) + #, xmin = -40.2, xmax = -38.2
  theme_bw() +
  ...
  #scale_linetype_manual(values = c(1,2,3), aesthetics = c("P", "PZ", "PZN")) +
    scale_x_continuous("Temperature (1/kTi)", sec.axis = sec_axis(~(-(1/(k*+.))-273), name = xlab)) +
  ylab("ln(umol O2 / L / hr)") +
  labs(title = "B. Net Ecosystem Respiration (ER)", size = 3) +
  annotate("text", x = c(-39,-39,-39), y = c(-6.5, -6.7, -6.9), label = c("solid = A", "dotted = AG", "dashed = AGP"), size = 4)

  ggsave("Fig3B.png", device = "png", width = 4, height = 4)
```       


## 4.5 ER temperature dependence within and across systems
```{r ER plot}
...
# 5. COMBINED RESULTS: Figure 5 (Full)

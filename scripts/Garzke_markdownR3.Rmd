...
kable(PPres, digits=2, col.names = c('Int','Z~j~','T~wj~','T~M~', 'T~wj~*T~M~', 'T~wj~*Z~j~','T~M~*Z~j~', 'df', 'logLik','AICc','d','w'))
```
...
```{r Table S2.3}
kable(NPres, digits=2, col.names = c('Int','Z~j~','T~wj~','T~M~', 'T~wj~*T~M~', 'T~wj~*Z~j~','T~wj~*Z~j~', 'df', 'logLik','AICc','d','w'))
```
...
NPP.plot <- ggplot(data = data1, aes(x = invTi, y = log(NPP2), ymin = -10.5, ymax = -4, xmin = 40.2, xmax = 38.2)) +
  theme_bw() +
  theme(legend.position = "none",
    panel.grid.minor = element_blank(), 
    panel.grid.major = element_blank(),
...
```{r Table S5}
kable(ERres, digits=2, col.names = c('Int','Z~j~','T~M~','T~wj~', 'T~wj~*T~M~', 'T~M~*Z~j~','T~M~*Z~j~', 'df', 'logLik','AICc','d','w'))
```
